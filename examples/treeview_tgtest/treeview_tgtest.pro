...
SOURCES += src/main.cpp \
                src/treeview_tgtest.cpp

HEADERS += src/treeview_tgtest.h

# cuuimake runs uic
# FORMS    = src/treeview_tgtest.ui
# but we need to include ui_xxxx.h file amongst the headers
# in order to be recompiled when it changes
#
HEADERS += \
    ui/ui_treeview_tgtest.h
...
!isEmpty(INSTALL_DIR) {
    ...
    message("       execute `make install` to install 'treeview_tgtest' under $${INSTALL_DIR} ")
    message("-")
} else {
    message("-")
    message("INSTALLATION")
    message("       call qmake INSTALL_DIR=/usr/local/bin to install treeview_tgtest later with `make install` ")
    message("-")
}
